...
class ObdEngine {
 ...
 suspend fun liveData(): String {
  val c = conn ?: error("Non connecté")
  val rr = c.run(RPMCommand(), useCache = false, maxRetries = 4)
  val tt = c.run(EngineCoolantTemperatureCommand(), useCache = false, maxRetries = 4)
  val rpmValue = rr.value.filter { it.isDigit() || it == '.' || it == '-' }.toDoubleOrNull()?.toInt()
  val tempValue = tt.value.filter { it.isDigit() || it == '.' || it == '-' }.toDoubleOrNull()?.toInt()
  last = last.copy(rpm = rpmValue, coolant = tempValue)
  return "DONNÉES ECU RÉELLES\\nRPM: "+rr.value+" "+rr.unit+"\\nLiquide refroidissement: "+tt.value+" "+tt.unit
 }
 fun snapshot()=last
